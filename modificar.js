...
function fetchProducts() {
    fetch('http://cartavirtual.azurewebsites.net/productos')
        .then((response) => {
...
productList.addEventListener('click', (event) => {
    if (event.target.classList.contains('modify-button')) {
        ...
        fetch(`http://cartavirtual.azurewebsites.net/productos/${updatedProduct.id}`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
...
productList.addEventListener('click', (event) => {
    if (event.target.classList.contains('delete-button')) {
        ...
        fetch(`http://cartavirtual.azurewebsites.net/productos/${productId}`, {
            method: 'DELETE',
        })
            .then((response) => {
